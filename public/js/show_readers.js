$("document").ready(function () {
  ...
  addSubmit.click(function (e) {
    ...
    const phoneNumber = $('.box-add.add input[name="phonenumber"]').val().trim();
    $.ajax({
      url: "../controller/readers/Readers.php",
      type: "post",
      data: {
  ...
  editSubmit.click(function (e) {
    ...
    const phoneNumber = $('.box-add.edit input[name="phonenumber"]').val().trim();
    $.ajax({
      url: "../controller/readers/Readers.php",
      type: "post",
      data: {
  ...
  $("tbody").on("click", ".edit-btn", function (e) {
    ...
    if (confirm("Are you sure you want to edit?")) {
      ...
      $.ajax({
        ...
        success: function (data) {
          const response = JSON.parse(data);
          $('.box-add.edit input[name="username"]').val(response[0]).trim();
          $('.box-add.edit input[name="fullname"]').val(response[1]).trim();
          $('.box-add.edit select[name="gender"]').val(response[2]).trim();
          $('.box-add.edit input[name="email"]').val(response[3]).trim();
          $('.box-add.edit input[name="address"]').val(response[4]).trim();
          $('.box-add.edit input[name="phonenumber"]').val(response[5]).trim();
        },
      });
    }
  });
