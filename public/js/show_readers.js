...
$("document").ready(function () {
    ...
    $(this).blur(function () {
      if ($(this).val() == "") {
        $(this)
          ...
          .find("p")
          .append(
            "<ion-icon name='alert-circle-sharp'></ion-icon>This field is required!"
          );
      } else {
        if (validate($(this).attr("validate"), $(this).val())) {
          $(this)
            .parent()
            .find("p")
            .append(validate($(this).attr("validate"), $(this).val()));
        }
      }
    });
  });
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data === "success") {
            toast({
              title: "Success",
              message: `Edit reader ${fullname}(${username}) successfully, please reload to see new information`,
              type: "success",
            });
          } else {
            toast({
  ...
  applyBtn.click(function (e) {
    ...
    const search_fullname = $('.search-box input[name="search_fullname"]')
      .val()
      .trim();
    const search_gender = $('.search-box select[name="search_gender"]').val();
    ...
    const search_email = $('.search-box input[name="search_email"]')
      .val()
      .trim();
    const search_phonenumber = $('.search-box input[name="search_phonenumber"]')
      .val()
      .trim();
        ...
        .each(function () {
          ...
          for (let i = 0; i < values.length; i++) {
            ...
            if (i == 2) {
              ...
              if (
                !$(td[i]).text().toLowerCase().includes(values[i].toLowerCase())
              ) {
                result = false;
              }
            }
